...
jQuery(document).ready(function($){
	...
	$(document).on('click', '#page-builder a.block-edit', function() {
		...
		$('.mce-edit-area').css('min-height','240px !important;');
		if( $('#' + blockID).hasClass('block-edit-active') == false ) {
			...
			if($('.block-edit-active').find('#shortcodes').hasClass('shortcodes')){
				...
				setTimeout(function(){ 
				  ...
				  jQuery('#'+blockID+' iframe').css('width','100%').css('minHeight','240px');
				   jQuery('#'+blockID+' iframe').contents().find("body").css('minHeight','200px');
				...
				tinymce.init({				
					...
					extended_valid_elements: "a[href|target|onclick|title]",					
...
jQuery(document).ready(function($){
	...
	$(document).on('click', '.aq_upload_button', function(event) {
		...
		frame.on( 'select', function() {
			...
			$('#'+blockID+' #' + input_id).val(attachment.attributes.url);
