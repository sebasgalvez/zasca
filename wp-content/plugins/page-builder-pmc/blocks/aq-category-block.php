...
class AQ_Category_Block extends AQ_Block {
	...
	function form($instance) {
		...
		if (pmc_woo() && PMC_SHOP){ 
			...
			<p class="description half last">		
				...
				<label for="<?php echo $this->get_field_id('show_sub') ?>" style="width:100%;float: left;margin-bottom: 10px;">
					<?php echo aq_field_checkbox('show_sub', $block_id, $show_sub); ?>
					Show subcategories?
				</label>				
	...
	function block($instance) {
		...
		if (pmc_woo() && PMC_SHOP){ 
			...
			extract($instance);
			echo $show_sub;
			if($show_sub == 0 ){$show_sub = 0; } else {$show_sub = ''; }
			$product_options = join(',', $categories_product);
			echo do_shortcode( stripslashes('[product_categories ids="'.$product_options.'"  parent="'.$show_sub.'" number="'.$number_product.'" columns="'.$rows_product.'" orderby="'.$product_sort_by.'" order="'.$product_order.'" hide_empty="'.$hide_empty.'" ]') );
